...
impl GridSliceMut<'_> {
    ...
    pub fn slice_mut<T, U>(&mut self, range_height: T, range_width: U) -> GridSliceMut where 
    ...
    pub fn get_bound(&self) -> Bound {
        Bound::new(self.heights.iter().sum::<usize>() - 1, self.widths.iter().sum::<usize>() - 1)
    }
}

impl GridSlice<'_> {

    pub fn slice<T, U>(&self, range_height: T, range_width: U) -> GridSlice where 
        T: SliceIndex<[usize], Output = [usize]>,
        U: SliceIndex<[usize], Output = [usize]>
        {
        GridSlice {
