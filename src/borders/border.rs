...
impl BorderFragment {
    fn new(length: usize, width: Width, left_start: Width, right_start: Width) -> BorderFragment {
        BorderFragment { length, width, left_start, right_start }
    }
    ...
    fn render_view(&self, start: &Width, orientation: Orientation) -> String {
        ...
        match orientation {
            Orientation::Vertical => {
                view.push(Width::draw_char(start, &self.left_start, &self.width, &self.right_start));
                for _ in 1..self.length {
                    view.push(Width::draw_char(&self.width, &Width::None, &self.width, &Width::None));
                }
            },
            Orientation::Horizontal => {
                view.push(Width::draw_char(&self.left_start, start, &self.right_start, &self.width));
                for _ in 1..self.length {
                    view.push(Width::draw_char(&Width::None, &self.width, &Width::None, &self.width));
                }
            }
        }
...
impl Border {
    ...
    pub fn combine<'a>(&'a self, other: &'a Border) -> Border {
        ...
        for BorderFragment { length, width, left_start, right_start } in self.fragments.iter() {
        ...
        let mut i: usize = 0;
        for BorderFragment { length, width, left_start, right_start } in other.fragments.iter() {
        ...
        for ch in chars {
            match ch {
                Combination { forward, left: &Width::None, right: &Width::None } if *forward == *prev => {
                    fragments.last_mut().unwrap().length += 1;
                }
                _ => {
                    fragments.push(BorderFragment::new(
                        1,
                        *ch.forward,
                        *ch.left,
                        *ch.right
...
impl CellBorder {
    pub fn new(left: Border, right: Border, top: Border, bottom: Border) -> CellBorder {
        if left.length != right.length || top.length != bottom.length {
            panic!("CellBorder: Inconsistent border length. Top: {}, bottom: {}, left: {}, right: {}",
                   top.length, bottom.length, left.length, right.length);
        }

        let height = left.length;
        let width = top.length;
        CellBorder { left, right, top, bottom, height, width }
    }
    ...
    pub fn render_view(&self, text: &Vec<String>) -> Vec<String> {
        ...
        for ((line, left), right) in text.iter()
...
mod tests {
    ...
    fn fragment_created() {
        ...
        assert_eq!("╶────", fragment.render_view(&Width::None, Orientation::Horizontal));
        assert_eq!("╷││││", fragment.render_view(&Width::None, Orientation::Vertical));
    }

    #[test]
    fn fragment_start_pieces() {
        let fragment = BorderFragment::new(5, Width::Light, Width::Light, Width::Heavy);
        assert_eq!("╅────", fragment.render_view(&Width::Heavy, Orientation::Horizontal));
        assert_eq!("╄││││", fragment.render_view(&Width::Heavy, Orientation::Vertical));
    }
}
